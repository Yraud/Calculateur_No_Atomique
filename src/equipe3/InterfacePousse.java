...
public class InterfacePousse {
	...
	private void initialize() {
		...
		frmCalcul.setTitle("Calculateur de numéro atomique");
		...
		btnValiderIon.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				int electronAtome1 = 0,electronAtome2 = 0, charge = 0, numIndice1 = 0, numIndice2 = 0, reponse;
				String nomIon = "", symboleAtome1 = "", symboleAtome2 = "", texteCharge = "", texteIndice1,texteIndice2;
				try {
					if (indiceAtome1.getText().equals("")) {
						numIndice1 = 1;
						texteIndice1 = "";
					} else {
						numIndice1 = Integer.parseInt(indiceAtome1.getText());
						texteIndice1 = numIndice1 + "";
					}
					if (!(zoneTexteAtome1.getText().equals(""))) {
						
						Element atome1 = Elements.getElement(zoneTexteAtome1.getText());
						electronAtome1 = atome1.getNombreElectrons()*numIndice1;
						symboleAtome1 = atome1.getSymbol();
					}
					if (indiceAtome2.getText().equals("")) {
						numIndice2 = 1;
						texteIndice2 = "";
					} else {
						numIndice2 = Integer.parseInt(indiceAtome2.getText());
						texteIndice2 = numIndice2 + "";
					}

					if (!(zoneTexteAtome2.getText().equals(""))) {

						Element atome2 = Elements.getElement(zoneTexteAtome2.getText());
						electronAtome2 = atome2.getNombreElectrons()*numIndice2;
						symboleAtome2 = atome2.getSymbol();
					}
					if (!(zoneTexteCharge.getText().equals(""))) {
						charge = Integer.parseInt(zoneTexteCharge.getText());
						if (charge < 0 ) {
							texteCharge = "(" + zoneTexteCharge.getText() + ")";
						} else {
							texteCharge = "(+" + zoneTexteCharge.getText() + ")";
						}
					}

					nomIon = symboleAtome1 + texteIndice1 + symboleAtome2 + texteIndice2 + texteCharge;


					reponse = JOptionPane.showConfirmDialog(internalFrame, "Votre ion est X" + nomIon + " . Appuyez sur oui pour passez au calcul du nom de l'élément X");

					if (reponse == 0) {
						electronSoustrait = electronAtome1 + electronAtome2 - charge;
						internalFrame.doDefaultCloseAction();
						zoneTexte.setVisible(true);
						zoneConsigne.setVisible(true);
						btnConfirmer.setVisible(true);
					}

				} catch (Exception error) {
					error.printStackTrace();
					JOptionPane.showMessageDialog(internalFrame, "Veuillez vous assurez que les informations sont correctes et réessayer. Il vous faut écrire ");
				}
